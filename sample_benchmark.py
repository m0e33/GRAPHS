...
def benchmark_neanet(Graph):
    ...
    results['max_wcc_percent'] = snap.GetMxWccSz(Graph) \
                                 / results['num_nodes']
    results['max_scc_percent'] = snap.GetMxSccSz(Graph) \
                                 / results['num_nodes']
...
def generate_graph(NNodes, NEdges, Model, Type, Rnd):
    if Model == 'rand_ungraph':
        ...
        Graph = snap.GenRndGnm(snap.PNEANet, NNodes, NEdges)
        print("2")
                ...
                for g in graph_types:
                    ...
                    if not generate:
                        ...
                        if os.path.exists(FName):
                            try:
                                ...
                                if verbose:
                                    print("Re-loaded graph with %d Nodes and %d Edges" % (Graph.GetNodes(), Graph.GetEdges()))
                    ...
                    if not Graph:
                        ...
                        try:
                            ...
                            if verbose:
                                print("Generating '%s %s' graph with %e nodes, %e edges..." % (Type, g, NNodes, NEdges))
                                sys.stdout.flush()
